...
func TestVerifyPostgreSQLRecoveryEvidenceAcceptsExactProof(t *testing.T) {
	payload := marshalPostgreSQLRecoveryEvidence(t, validPostgreSQLRecoveryEvidence())
	if err := VerifyPostgreSQLRecoveryEvidence(bytes.NewReader(payload)); err != nil {
		t.Fatalf("exact recovery evidence was rejected: %v", err)
	}
}

func TestVerifyPostgreSQLRecoveryEvidenceRejectsAdversarialProofs(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			test.mutate(&value)
			if err := VerifyPostgreSQLRecoveryEvidence(bytes.NewReader(marshalPostgreSQLRecoveryEvidence(t, value))); err == nil {
...
func TestVerifyPostgreSQLRecoveryEvidenceRejectsMissingZeroAndNullFields(t *testing.T) {
	tests := []struct {
		name string
		path []any
	}{
...
func TestVerifyPostgreSQLRecoveryEvidenceRejectsDuplicateAndUnknownFields(t *testing.T) {
	payload := marshalPostgreSQLRecoveryEvidence(t, validPostgreSQLRecoveryEvidence())
	duplicate := bytes.Replace(payload, []byte(`"verdict":"pass"`), []byte(`"verdict":"pass","verdict":"pass"`), 1)
	if err := VerifyPostgreSQLRecoveryEvidence(bytes.NewReader(duplicate)); err == nil {
	...
	if err := VerifyPostgreSQLRecoveryEvidence(bytes.NewReader(unknown)); err == nil {
...
func validPostgreSQLRecoveryEvidence() postgresqlRecoveryEvidence {
	...
	return postgresqlRecoveryEvidence{
		SchemaVersion:  postgresqlRecoveryEvidenceSchemaVersion,
